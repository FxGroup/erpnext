...
def get_statement_dict(doc, get_statement_dict=False):
    ...
    outstanding_docs = set_outstanding(doc, party_list)
    outstanding = []
    
    for voucher in outstanding_docs:
    ...
    for idx, entry in enumerate(doc.customers, 1):
        logger.info(f"[Get Statement Dict][{idx}/{len(doc.customers)}] Processing customer: {entry.customer}")
        ...
        if doc.report == "General Ledger":
            ...
            if not customer_res:
                ...
                if abs(customer_outstanding) < 0.01 and outstanding_docs:
                    for voucher in outstanding_docs:
    ...
    if report:
        ...
        for customer, report_pdf in report.items():
            ...
            if not frappe.conf.production_site:
                recipients = "it@fxmed.co.nz"
                logger.info("[Send Email] TESTING MODE: Updating Customer email address to: {}".format(recipients))
                cc = []
            ...
            enqueue_args = {
                ...
                "subject":subject,
                "message":message,
                "is_async": bool(frappe.conf.production_site),
                "reference_doctype":"Process Statement Of Accounts",
                "reference_name":document_name,
...
def send_auto_email():
    ...
    logger.info("[Send Auto Email] Found {} Process Statement of Accounts documents with schedule_send enabled".format(len(selected)))
